...
// LLVM 的 `AffineExpr.h` 是用于定义和处理仿射表达式的，这在 LLVM 的多维循环优化和数
// 组访问中是非常重要的。仿射表达式主要用于表示多维循环中索引的线性变换和常数偏移，这对
// 于循环优化、代码生成以及对多维数据结构的访问分析等方面至关重要。 MLIR 的 affine 方
// 言使用这些仿射表达式来处理仿射变换和操作，常用于循环优化。所以， `AffineExpr.h` 与
// MLIR 的 affine 方言之间的关系在于，后者依赖于前者提供的仿射表达式和相关功能，以实现
// 其在循环优化和数组访问中的应用。
